...
maxlag = 100;
RF = estimateResponseFunction(N,Ninf_SR,maxlag); % infer response functions.
RF_no_SR = estimateResponseFunction(N,Ninf_no_SR,maxlag); % infer response functions.
figure;
plot([-maxlag:maxlag] * O.dt, RF,'LineWidth',2); hold on;
plot([-maxlag:maxlag] * O.dt, RF_no_SR,'LineWidth',2);
xlim([-0.1,0.1]);
...
maxlag = 100;
RF_blind = estimateResponseFunction(N,Ninf_SR,maxlag); % infer response functions.
RF_no_SR_blind = estimateResponseFunction(N,Ninf_no_SR,maxlag); % infer response functions.
figure;
plot([-maxlag:maxlag] * O.dt, RF,'LineWidth',2); hold on;
plot([-maxlag:maxlag] * O.dt, RF_no_SR,'LineWidth',2);
