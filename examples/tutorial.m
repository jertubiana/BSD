...
%% ~~~~~ Typical use case: Suppose the rise and decay time scales are known.
...
%% ~~~~~ Typical use case: Ease kernel inference by providing bounds.
% In situations where: i) the spike count is low and/or ii) the recording
% contains mostly burst of spikes and/or iii) the SNR is small.
% The inference of rise and decay time can be challenging. Can "help" the algorithm by providing known boundaries on rise and decay time constants.
...
P = struct; % Struct of generative model properties.
P.tauRise = 0.1; % Fluorescence rise time (s)
P.tauDecay = 0.5; % Fluorescence decay time (s)
P.nu = 0.5; % Neuron firing rate (Hz).
P.a = 1; % Spike amplitude
P.b = 0; % Baseline position
...
Oalg = O;
Oalg.adaptive = 1; % Adaptive; => iterate over b
Oalg.tauRiseMin = 0.05; % Optional constraint.
Oalg.tauRiseMax = 0.2; % Optional constraint.
% Oalg.tauDecayMin = 0.25; % Optional constraint.
Oalg.tauDecayMax = 1.0; % Optional constraint.
